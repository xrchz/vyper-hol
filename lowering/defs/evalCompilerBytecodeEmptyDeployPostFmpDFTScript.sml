Theory evalCompilerBytecodeEmptyDeployPostFmpDFT
Ancestors evalCompilerBytecodeEmptyDeployPostFmpSingleUseExpansion
...
val incoming =
  evalCompilerBytecodeEmptyDeployPostFmpSingleUseExpansionTheory.exact_empty_deploy_after_post_fmp_single_use_expansion
...
val _ = assert_closed "DFT unit option case" unit_case_tm
...
val _ = assert_closed "DFT result option case" result_case_tm
...
val after_dft = PURE_REWRITE_RULE [result_case] context_exposed
...
val _ =
  if not (null next_passes) andalso
     aconv (hd next_passes) ``CFP_Simple VP_CFGNormalization``
  then () else raise Fail "post-FMP deploy fold is not headed by CFGNormalization"
val _ = assert_closed "post-FMP deploy after DFT" (concl after_dft)

Theorem exact_empty_deploy_after_post_fmp_dft:
  ^(concl after_dft)
Proof
