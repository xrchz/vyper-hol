Theory evalCompilerBytecodeEmptyDeploySimplifyCfgIter
Ancestors evalCompilerBytecodeEmptyDeploySimplifyCfgRound
...
val round_fixpoint_guard_th = computeLib.EVAL_CONV
  ``empty_deploy_simplify_cfg_round1_fn.fn_blocks =
    empty_deploy_simplify_cfg_operand.fn_blocks``

Theorem exact_empty_deploy_round_fixpoint_guard:
...
val iter_suc_clause =
  Q.SPECL [`0`, `empty_deploy_simplify_cfg_operand`]
    (cj 2 simplifyCfgDefsTheory.simplify_cfg_iter_with_labels_def)
val exact_iter_th = SIMP_RULE (srw_ss ())
  [evalCompilerBytecodeEmptyDeploySimplifyCfgRoundTheory.exact_empty_deploy_simplify_cfg_round_with_labels,
...
val _ =
  ...
  then raise Fail "deploy one-step iterator left a recursive iterator"
  else ()

Theorem exact_empty_deploy_simplify_cfg_iter_with_labels:
  ^(concl exact_iter_th)
Proof
...
val fn_unfold = REWR_CONV
  ...
  ``simplify_cfg_fn_with_labels empty_deploy_simplify_cfg_operand``
val exact_fn_th = SIMP_RULE (srw_ss ())
  [evalCompilerBytecodeEmptyDeploySimplifyCfgTheory.exact_empty_deploy_simplify_cfg_operand_count,
...
val _ =
  ...
  then raise Fail "deploy SimplifyCFG wrapper left a recursive head"
  else ()

Theorem exact_empty_deploy_simplify_cfg_fn_with_labels:
  ^(concl exact_fn_th)
Proof
