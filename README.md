...
come from the pinned revision. The machine-readable policy for this export is
[`tests/vyper-test-export-settings.json`](tests/vyper-test-export-settings.json),
under the profile name `functional_test_exports`. Individual upstream tests may
override compiler settings, so each exported JSON file's `compiler_settings`
remain authoritative for that test. HOL currently trusts those exported settings
and consumes or checks only part of them; the manifest records the export default,
not an additional HOL validation guarantee.

The separate
[`formal_o1_ir_no_asm_opt`](lowering/defs/bytecode/fixture-settings.json) profile
covers bytecode generated by the formal compiler for HOL self-regression. It uses
the revision named by `VYPER_PIN`, formal O1 IR optimization, and no assembly
