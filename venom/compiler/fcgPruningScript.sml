...
Definition pruning_eval_unit_def[local]:
  pruning_eval_unit = <|
    cu_context :=
      (mk_venom_context
        [mk_raw_function "keep_a" [];
         mk_raw_function "drop_middle" [];
         mk_raw_function "keep_b" []]
        (SOME "keep_a")) with
      ctx_global_reserved := [(4, 8)];
    cu_data_segment :=
      [<| ds_label := "mixed_data";
          ds_items := [DataLabel "drop_middle_label";
                       DataBytes [1w; 2w]] |>]
  |>
End

Definition pruning_eval_fcg_def[local]:
  pruning_eval_fcg =
    fcg_empty with fcg_reachable := ["keep_a"; "keep_b"]
End

Theorem prune_fcg_mixed_eval:
  MAP (\fn. fn.fn_name)
      (prune_unit_fcg_unreachable
        pruning_eval_unit pruning_eval_fcg).cu_context.ctx_functions =
    ["keep_a"; "keep_b"] /\
  (prune_unit_fcg_unreachable
    pruning_eval_unit pruning_eval_fcg).cu_data_segment =
    pruning_eval_unit.cu_data_segment /\
  (prune_unit_fcg_unreachable
    pruning_eval_unit pruning_eval_fcg).cu_context.ctx_entry =
    pruning_eval_unit.cu_context.ctx_entry /\
  (prune_unit_fcg_unreachable
    pruning_eval_unit pruning_eval_fcg).cu_context.ctx_global_reserved =
    pruning_eval_unit.cu_context.ctx_global_reserved
Proof
  EVAL_TAC
QED
