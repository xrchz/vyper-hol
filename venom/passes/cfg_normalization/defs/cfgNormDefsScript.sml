...
(* Two functions both need a critical-edge split.  The unit already contains
   the first supply-style label/variable, a legacy split label and forwarding
   variable, and a sparse high instruction ID. *)
Definition cfg_norm_supply_collision_probe_unit_def:
  cfg_norm_supply_collision_probe_unit = <|
    cu_context := mk_venom_context
      [mk_raw_function "probe_cfg_f1"
         [<| bb_label := "P1"; bb_instructions :=
               [mk_inst 7 JNZ
                  [Var "c1"; Label "B1"; Label "C1"] []] |>;
          <| bb_label := "C1"; bb_instructions :=
               [mk_inst 8 JMP [Label "B1"] []] |>;
          <| bb_label := "B1"; bb_instructions :=
               [mk_inst 9 PHI
                  [Label "P1"; Var "x1"; Label "C1"; Var "z1"] ["y1"];
                mk_inst 10 STOP [] []] |>;
          <| bb_label := "formal_label_0"; bb_instructions :=
               [mk_inst 9000 STOP [] ["formal_var_0"]] |>;
          <| bb_label := "P1_split_B1"; bb_instructions :=
               [mk_inst 11 STOP [] ["P1_split_B1_fwd_x1"]] |>];
       mk_raw_function "probe_cfg_f2"
         [<| bb_label := "P2"; bb_instructions :=
               [mk_inst 20 JNZ
                  [Var "c2"; Label "B2"; Label "C2"] []] |>;
          <| bb_label := "C2"; bb_instructions :=
               [mk_inst 21 JMP [Label "B2"] []] |>;
          <| bb_label := "B2"; bb_instructions :=
               [mk_inst 22 PHI
                  [Label "P2"; Var "x2"; Label "C2"; Var "z2"] ["y2"];
                mk_inst 23 STOP [] []] |>]]
      (SOME "probe_cfg_f1");
    cu_data_segment := [] |>
End

Theorem cfg_norm_supply_collision_probe:
  let result = cfg_norm_configured_with_supply
                 cfg_norm_supply_collision_probe_unit in
    (SND result).irs_next_inst = 9005 /\
    (SND result).irs_next_var = 3 /\
    (SND result).irs_next_label = 3 /\
    TAKE 4 (SND result).irs_used_inst_ids = [9004;9003;9002;9001] /\
    TAKE 2 (SND result).irs_used_vars = ["formal_var_2";"formal_var_1"] /\
    TAKE 2 (SND result).irs_used_labels =
