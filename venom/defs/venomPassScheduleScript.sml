...
(* Closed probes for the generic constraint boundary.  Each synthetic record
 * activates exactly one of the five fields. *)
Theorem pass_constraints_predecessor_probe:
  pass_constraints_hold
    (no_pass_constraints with pc_predecessors := [VP_MakeSSA])
    [VP_MakeSSA] [] /\
  ~pass_constraints_hold
    (no_pass_constraints with pc_predecessors := [VP_MakeSSA])
    [] []
...
Theorem pass_constraints_successor_probe:
  pass_constraints_hold
    (no_pass_constraints with pc_successors := [VP_MakeSSA])
    [] [VP_MakeSSA] /\
  ~pass_constraints_hold
    (no_pass_constraints with pc_successors := [VP_MakeSSA])
    [] []
...
Theorem pass_constraints_immediate_predecessor_probe:
  pass_constraints_hold
    (no_pass_constraints with
       pc_immediate_predecessors := [VP_MakeSSA])
    [VP_LowerDload; VP_MakeSSA] [] /\
  ~pass_constraints_hold
    (no_pass_constraints with
       pc_immediate_predecessors := [VP_MakeSSA])
    [VP_MakeSSA; VP_LowerDload] []
Proof
  EVAL_TAC
QED

Theorem pass_constraints_immediate_successor_probe:
  pass_constraints_hold
    (no_pass_constraints with
       pc_immediate_successors := [VP_MakeSSA])
    [] [VP_MakeSSA; VP_LowerDload] /\
  ~pass_constraints_hold
    (no_pass_constraints with
       pc_immediate_successors := [VP_MakeSSA])
...
Theorem pass_constraints_ordered_after_probe:
  pass_constraints_hold
    (no_pass_constraints with pc_ordered_after := [VP_MakeSSA])
    [] [VP_LowerDload] /\
  pass_constraints_hold
    (no_pass_constraints with pc_ordered_after := [VP_MakeSSA])
    [VP_MakeSSA] [VP_LowerDload] /\
