...
(* Small executable probes for the transaction boundary. *)
Definition task039_policy_def:
  task039_policy = <|
...
Definition task039_duplicate_unit_def:
  task039_duplicate_unit = <|
...
Definition task039_identity_runner_def:
  task039_identity_runner rpolicy pass unit s fn =
    SOME <| fpo_function := fn; fpo_label_map := []; fpo_supply := s |>
End

Definition task039_invalid_effect_runner_def:
  task039_invalid_effect_runner rpolicy pass unit s fn =
    SOME <| fpo_function := fn with fn_eom := SOME 0;
            fpo_label_map := []; fpo_supply := s |>
End

Definition task039_stale_supply_runner_def:
  task039_stale_supply_runner rpolicy pass unit s fn =
    SOME <| fpo_function := fn; fpo_label_map := [];
            fpo_supply := s with irs_next_inst := 0 |>
End

Definition task039_map_runner_def:
  task039_map_runner label_map rpolicy pass unit s fn =
    SOME <| fpo_function := fn; fpo_label_map := label_map;
            fpo_supply := s |>
End

Theorem run_fn_schedule_empty_eval:
  run_fn_schedule task039_policy [] "f" (task009_label_fixture "old") =
    SOME (task009_label_fixture "old",
          init_ir_supply (task009_label_fixture "old"))
Proof
  EVAL_TAC
QED

Theorem run_configured_fn_passes_duplicate_eval:
...
Theorem run_configured_fn_passes_missing_eval:
...
Theorem run_configured_fn_passes_invalid_effect_eval:
...
Theorem run_configured_fn_passes_stale_supply_eval:
...
Theorem run_configured_fn_passes_stale_input_supply_eval:
  run_configured_fn_passes task039_identity_runner task039_policy [] "f"
    (task009_label_fixture "old")
    ((init_ir_supply (task009_label_fixture "old")) with
       irs_used_inst_ids := []) = NONE
Proof
  EVAL_TAC
QED

Theorem run_configured_fn_passes_bad_maps_eval:
  run_configured_fn_passes
    (task039_map_runner [("old","mid");("mid","old")]) task039_policy
    [CFP_Simple VP_MakeSSA] "f" (task009_label_fixture "old")
    (init_ir_supply (task009_label_fixture "old")) = NONE /\
  run_configured_fn_passes
    (task039_map_runner [("old","mid");("old","new")]) task039_policy
    [CFP_Simple VP_MakeSSA] "f" (task009_label_fixture "old")
    (init_ir_supply (task009_label_fixture "old")) = NONE /\
  run_configured_fn_passes
